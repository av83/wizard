        ...
        (acts (list
               (list :perm ':ALL
                     ...
                     :fields (list
                              ...
                              (list :btn "B1376" :perm 111 :value "Показать ресурсы"))))))
    (show-acts acts)))

(restas:define-route catalog-page/post ("/catalog" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
                ("B1376" . ,(lambda () (TO "/category/~A" (CAAR (FORM-DATA))))))))
    (activate acts)))
  ...
  (let ((session (hunchentoot:start-session))
        (acts (list
               (list :perm ':ALL
                     ...
                     :fields (list
                              (list :fld "NAME" :perm 111 :typedata '(:STR) :name "Имя")
                              (list :btn "B1377" :perm 111 :value "Показать ресурсы")))
               (list :perm ':ALL
                     ...
                     :fields (list
                              ...
                              (list :btn "B1378" :perm 111 :value "Страница ресурса"))))))
    (show-acts acts)))

(restas:define-route category-page/post ("/category/:id" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
                ("B1377" . ,(lambda () (TO "/category/~A" (CAAR (FORM-DATA)))))
                ("B1378" . ,(lambda () (TO "/resource/~A" (CAAR (FORM-DATA))))))))
    (activate acts)))
        ...
        (acts (list
               (list :perm ':ALL
                     ...
                     :fields (list
                              ...
                              (list :fld "RESOURCE-TYPE" :perm 111 :typedata '(:LIST-OF-KEYS RESOURCE-TYPES) :name "Тип")
                              (list :fld "UNIT" :perm 111 :typedata '(:STR) :name "Единица измерения")
                              (list :btn "B1379" :perm 111 :value "Страница категории")
                              (list :btn "B1380" :perm 111 :value "Страница ресурса"))))))
    (show-acts acts)))

(restas:define-route resources-page/post ("/resource" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
                ("B1379" . ,(lambda () (HUNCHENTOOT:REDIRECT
                ...
                ("B1380" . ,(lambda () (TO "/resource/~A" (CAAR (FORM-DATA))))))))
    (activate acts)))
        ...
        (acts (list
               (list :perm ':ADMIN
                     ...
                     :fields (list
                              ...
                              (list :btn "B1381" :perm 111 :value "Изменить пароль")))
               (list :perm ':ADMIN
                     ...
                     :fields (list
                              ...
                              (list :btn "B1382" :perm 111 :value "Создать новый аккаунт эксперта")))
               (list :perm ':ADMIN
                     :title "Эксперты"
                     ...
                     :fields (list
                              ...
                              (list :popbtn "P1383"
                                    ...
                                    :fields (list
                                             (list :btn "B1384" :perm 111 :value "Подтверждаю удаление")))
                              (list :popbtn "P1385"
                                    ...
                                    :fields (list
                                             (list :fld "PASSWORD" :perm 111 :typedata '(:PSWD) :name "Пароль")
                                             (list :btn "B1386" :perm 111 :value "Изменить пароль эксперта")))
                              (list :btn "B1387" :perm 111 :value "Страница эксперта")))
               (list :perm ':ADMIN
                     ...
                     :fields (list
                              ...
                              (list :popbtn "P1388"
                                    ...
                                    :fields (list
                                             (list :btn "B1389" :perm 111 :value "Сделать добросовестным"))))))))
    (show-acts acts)))

(restas:define-route admin-page/post ("/admin" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
                ("B1381" . ,(lambda () (LET ((OBJ (CUR-USER)))
                ...
                ("B1384" . ,(lambda () (LET ((KEY (GET-BTN-KEY (CAAR (FORM-DATA)))))
                                         (REMHASH KEY *USER*)
                                         (HUNCHENTOOT:REDIRECT (HUNCHENTOOT:REQUEST-URI*)))))
                ("B1386" . ,(lambda () (LET ((OBJ (GETHASH (GET-BTN-KEY (CAAR (LAST (FORM-DATA)))) *USER*)))
                                         (WITH-OBJ-SAVE OBJ PASSWORD))))
                ("B1387" . ,(lambda () (TO "/expert/~A" (CAAR (FORM-DATA)))))
                ("B1389" . ,(lambda () (LET ((KEY (GET-BTN-KEY (CAAR (FORM-DATA)))))
        ...
        (acts (list
               (list :perm ':ALL
                     ...
                     :fields (list
                              (list :fld "NAME" :perm 111 :typedata '(:STR) :name "ФИО")
                              (list :fld "LOGIN" :perm 111 :typedata '(:STR) :name "Логин")
                              (list :btn "B1390" :perm 111 :value "Страница эксперта"))))))
    (show-acts acts)))

(restas:define-route experts-page/post ("/expert" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
                ("B1390" . ,(lambda () (TO "/expert/~A" (CAAR (FORM-DATA))))))))
    (activate acts)))
        ...
        (acts (list
               (list :perm ':ALL
                     ...
                     :fields (list
                              (list :fld "NAME" :perm 111 :typedata '(:STR) :name "Название организации")
                              (list :fld "LOGIN" :perm 111 :typedata '(:STR) :name "Логин")
                              (list :btn "B1391" :perm 111 :value "Страница поставщика"))))))
    (show-acts acts)))

(restas:define-route suppliers-page/post ("/supplier" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
                ("B1391" . ,(lambda () (TO "/supplier/~A" (CAAR (FORM-DATA))))))))
    (activate acts)))
        ...
        (acts (list
               (list :perm ':ADMIN
                     ...
                     :fields (list
                              (list :fld "LOGIN" :perm 111 :typedata '(:STR) :name "Логин")
                              (list :fld "PASSWORD" :perm 111 :typedata '(:PSWD) :name "Пароль")
                              (list :btn "B1392" :perm 111 :value "Изменить пароль")))
               (list :perm 'NIL
                     :title "Поставщик"
                     :val (lambda () (GETHASH (CUR-ID) *USER*))
                     :fields (list
                              ...
                              (list :btn "B1393" :perm 111 :value "Сохранить")
                              (list :col "Список поставляемых ресурсов" :perm 111
                                    ...
                                    :fields (list
                                             ...
                                             (list :popbtn "P1394"
                                                   ...
                                                   :fields (list
                                                            (list :btn "B1395" :perm 111 :value "Удалить ресурс")))))
                              (list :popbtn "P1396"
                                    :value "Добавить ресурс"
                                    :perm 111
                                    :title "Добавление ресурса"
                                    :fields (list
                                             (list :btn "B1397" :perm 111 :value "Добавить ресурс")))
                              (list :col "Список заявок на тендеры" :perm 111
                                    ...
                                    :fields (list
                                             ...
                                             (list :btn "B1398" :perm 111 :value "Страница заявки")
                                             (list :popbtn "P1399"
                                                   :value "Удалить заявку"
                                                   :perm 111
                                                   :title "Удаление заявки"
                                                   :fields (list
                                                            (list :btn "B1400" :perm 111 :value "Удалить заявку")))))
                              (list :col "Список распродаж" :perm 111
                                    ...
                                    :fields (list
                                             ...
                                             (list :btn "B1401" :perm 111 :value "Страница распродажи")
                                             (list :popbtn "P1402"
                                                   :value "Удалить распродажу"
                                                   :perm 111
                                                   :title "Удаление распродажи"
                                                   :fields (list
                                                            (list :btn "B1403" :perm 111 :value "Удалить распродажу")))))
                              (list :popbtn "P1404"
                                    :value "Добавить распродажу"
                                    :perm 111
                                    :title "Добавление расподажи"
                                    :fields (list
                                             (list :btn "B1405" :perm 111 :value "Добавить распродажу")))))
               (list :perm '(AND :SELF :UNFAIR)
                     :title "Отправить заявку на добросовестность"
                     :val (lambda () (GETHASH (CUR-ID) *USER*))
                     :fields (list
                              (list :btn "B1406" :perm 111 :value "Отправить заявку на добросовестность"))))))
    (show-acts acts)))

(restas:define-route supplier-page/post ("/supplier/:id" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
                ("B1392" . ,(lambda () (LET ((OBJ (CUR-USER)))
                                         (WITH-OBJ-SAVE OBJ LOGIN PASSWORD))))
                ("B1393" . ,(lambda () (LET ((OBJ (GETHASH (CUR-ID) *USER*)))
                                         ...
                                         (HUNCHENTOOT:REDIRECT (HUNCHENTOOT:REQUEST-URI*)))))
                ("B1395" . ,(lambda () (DEL-INNER-OBJ (CAAR (FORM-DATA)) *SUPPLIER-RESOURCE-PRICE*
                ...
                ("B1397" . ,(lambda () (PROGN
                                         ...
                                         (HUNCHENTOOT:REDIRECT (HUNCHENTOOT:REQUEST-URI*)))))
                ("B1398" . ,(lambda () (TO "/offer/~A" (CAAR (FORM-DATA)))))
                ("B1400" . ,(lambda () (DEL-INNER-OBJ (CAAR (FORM-DATA)) *OFFER* (A-OFFERS (GETHASH (CUR-ID) *USER*)))))
                ("B1401" . ,(lambda () (TO "/sale/~A" (CAAR (FORM-DATA)))))
                ("B1403" . ,(lambda () (DEL-INNER-OBJ (CAAR (FORM-DATA)) *SALE* (A-SALES (GETHASH (CUR-ID) *USER*)))))
                ("B1405" . ,(lambda () (CREATE-SALE)))
                ("B1406" . ,(lambda () (PROGN
  ...
  (let ((session (hunchentoot:start-session))
        (acts (list
               (list :perm ':ALL
                     ...
                     :fields (list
                              (list :fld "NAME" :perm 111 :typedata '(:STR) :name "Распродажа")
                              (list :btn "B1407" :perm 111 :value "Страница распродажи"))))))
    (show-acts acts)))

(restas:define-route sales-page/post ("/sale" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
                ("B1407" . ,(lambda () (TO "/sale/~A" (CAAR (FORM-DATA))))))))
    (activate acts)))
        ...
        (acts (list
               (list :perm 'NIL
                     ...
                     :fields (list
                              ...
                              (list :btn "B1408" :perm 111 :value "Сохранить")
                              (list :btn "B1409" :perm 111 :value "Удалить распродажу"))))))
    (show-acts acts)))

(restas:define-route sale-page/post ("/sale/:id" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
                ("B1408" . ,(lambda () (SAVE-SALE)))
                ("B1409" . ,(lambda () (DELETE-SALE))))))
    (activate acts)))
        ...
        (acts (list
               (list :perm ':ALL
                     ...
                     :fields (list
                              ...
                              (list :fld "LOGIN" :perm 111 :typedata '(:STR) :name "Логин")
                              (list :btn "B1410" :perm 111 :value "Страница застройщика"))))))
    (show-acts acts)))

(restas:define-route builders-page/post ("/builder" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
                ("B1410" . ,(lambda () (TO "/builder/~A" (CAAR (FORM-DATA))))))))
    (activate acts)))
        ...
        (acts (list
               (list :perm 'NIL
                     ...
                     :fields (list
                              ...
                              (list :btn "B1411" :perm 111 :value "Сохранить")
                              (list :col "Тендеры застройщика" :perm 111
                                    ...
                                    :fields (list
                                             ...
                                             (list :btn "B1412" :perm 111 :value "Страница тендера")))))
               (list :perm ':SELF
                     ...
                     :fields (list
                              ...
                              (list :btn "B1413" :perm 111 :value "Объявить тендер (+)"))))))
    (show-acts acts)))

(restas:define-route builder-page/post ("/builder/:id" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
                ("B1411" . ,(lambda () (LET ((OBJ (GETHASH (CUR-ID) *USER*)))
                                         (WITH-OBJ-SAVE OBJ NAME JURIDICAL-ADDRESS INN KPP OGRN BANK-NAME BIK
                                                        CORRESP-ACCOUNT CLIENT-ACCOUNT RATING))))
                ("B1412" . ,(lambda () (TO "/tender/~A" (CAAR (LAST (FORM-DATA))))))
                ("B1413" . ,(lambda () (LET ((ID (HASH-TABLE-COUNT *TENDER*)))
        ...
        (acts (list
               (list :perm ':ALL
                     ...
                     :fields (list
                              ...
                              (list :btn "B1414" :perm 111 :value "Страница тендера"))))))
    (show-acts acts)))

(restas:define-route tenders-page/post ("/tender" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
                ("B1414" . ,(lambda () (TO "/tender/~A" (CAAR (FORM-DATA))))))))
    (activate acts)))
        ...
        (acts (list
               (list :perm 'NIL
                     ...
                     :fields (list
                              ...
                              (list :fld "INTERVIEW" :perm 111 :typedata '(:INTERVAL) :name "Срок проведения интервью")
                              (list :fld "RESULT" :perm 111 :typedata '(:INTERVAL) :name "Срок подведения итогов")
                              (list :btn "B1415" :perm 111 :value "Сохранить")
                              (list :col "Ресурсы тендера" :perm 111
                                    ...
                                    :fields (list
                                             (list :fld "NAME" :perm 111 :typedata '(:STR) :name "Название")
                                             (list :btn "B1416" :perm 111 :value "Удалить из тендера")
                                             (list :btn "B1417" :perm 111 :value "Страница ресурса")))
                              (list :popbtn "P1418"
                                    :value "Добавить ресурс"
                                    :perm 111
                                    ...
                                    :fields (list
                                             (list :col "Выберите ресурс" :perm 111
                                                   ...
                                                   :fields (list
                                                            (list :fld "NAME" :perm 111 :typedata '(:STR) :name "Наименование")
                                                            (list :btn "B1419" :perm 111 :value "Добавить ресурс")))))
                              (list :col "Документы тендера" :perm 111
                                    ...
                                    :fields (list
                                             (list :fld "NAME" :perm 111 :typedata '(:STR) :name "Название")
                                             (list :btn "B1420" :perm 111 :value "Удалить из тендера")
                                             (list :btn "B1421" :perm 111 :value "Страница документа")))
                              (list :popbtn "P1422"
                                    ...
                                    :fields (list
                                             (list :col "Выберите ресурс" :perm 111
                                                   ...
                                                   :fields (list
                                                            (list :fld "NAME" :perm 111 :typedata '(:STR) :name "Наименование")
                                                            (list :btn "B1423" :perm 111 :value "Добавить ресурс")))))
                              (list :col "Поставщики ресурсов" :perm 111
                                    ...
                                    :fields (list
                                             (list :fld "NAME" :perm 111 :typedata '(:STR) :name "Название")
                                             (list :btn "B1424" :perm 111 :value "Отправить приглашение")
                                             (list :btn "B1425" :perm 111 :value "Страница поставщика")))
                              (list :btn "B1426" :perm 111 :value "Добавить своего поставщика")
                              (list :col "Заявки на тендер" :perm 111
                                    :val (lambda () (CONS-INNER-OBJS *OFFER*
                                                                     (A-OFFERS
                                                                       ...
                                                                       *TENDER*))))
                                    :fields (list
                                             (list :btn "B1427" :perm 111 :value "Страница заявки")))
                              (list :popbtn "P1428"
                                    :value "Ответить заявкой на тендер"
                                    :perm 111
                                    :title "Выберите ресурсы"
                                    :fields (list
                                             (list :btn "B1429" :perm 111 :value "Участвовать в тендере")))
                              (list :popbtn "P1430"
                                    :value "Отменить тендер"
                                    :perm 111
                                    :title "Действительно отменить?"
                                    :fields (list
                                             (list :btn "B1431" :perm 111 :value "Подтверждаю отмену"))))))))
    (show-acts acts)))

(restas:define-route tender-page/post ("/tender/:id" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
                ("B1415" . ,(lambda () (LET ((OBJ (GETHASH (CUR-ID) *TENDER*)))
                                         (WITH-OBJ-SAVE OBJ NAME ACTIVE-DATE ALL CLAIM ANALIZE INTERVIEW RESULT))))
                ("B1416" . ,(lambda () (LET ((ETALON (GETHASH (GET-BTN-KEY (CAAR (LAST (FORM-DATA)))) *RESOURCE*)))
                                         (SETF (A-RESOURCES (GETHASH (CUR-ID) *TENDER*))
                                               (REMOVE-IF #'(LAMBDA (X) (EQUAL X ETALON))
                                                          (A-RESOURCES (GETHASH (CUR-ID) *TENDER*))))
                                         (HUNCHENTOOT:REDIRECT (HUNCHENTOOT:REQUEST-URI*)))))
                ("B1417" . ,(lambda () (TO "/resource/~A" (CAAR (LAST (FORM-DATA))))))
                ("B1419" . ,(lambda () (PROGN
                                         (PUSH (GETHASH (GET-BTN-KEY (CAAR (LAST (FORM-DATA)))) *RESOURCE*)
                                               (A-RESOURCES (GETHASH (CUR-ID) *TENDER*)))
                                         (HUNCHENTOOT:REDIRECT (HUNCHENTOOT:REQUEST-URI*)))))
                ("B1420" . ,(lambda () (DELETE-DOC-FROM-TENDER)))
                ("B1421" . ,(lambda () (TO "/document/~A" (CAAR (LAST (FORM-DATA))))))
                ("B1423" . ,(lambda () (PROGN
                                         (PUSH (GETHASH (GET-BTN-KEY (CAAR (LAST (FORM-DATA)))) *RESOURCE*)
                                               (A-RESOURCES (GETHASH (CUR-ID) *TENDER*)))
                                         (HUNCHENTOOT:REDIRECT (HUNCHENTOOT:REQUEST-URI*)))))
                ("B1424" . ,(lambda () (DELETE-FROM-TENDER)))
                ("B1425" . ,(lambda () (TO "/supplier/~A" (CAAR (LAST (FORM-DATA))))))
                ("B1426" . ,(lambda () (ADD-DOCUMENT-TO-TENDER)))
                ("B1427" . ,(lambda () (TO "/offer/~A" (CAAR (LAST (FORM-DATA))))))
                ("B1429" . ,(lambda () (LET* ((ID (HASH-TABLE-COUNT *OFFER*))
                                              (OFFER
                ...
                ("B1431" . ,(lambda () (HUNCHENTOOT:REDIRECT (FORMAT NIL "/tender")))))))
    (activate acts)))
        ...
        (acts (list
               (list :perm ':ALL
                     ...
                     :fields (list
                              ...
                              (list :fld "TENDER" :perm 111 :typedata '(:LINK TENDER) :name "Тендер")
                              (list :btn "B1432" :perm 111 :value "Страница заявки"))))))
    (show-acts acts)))

(restas:define-route offers-page/post ("/offers" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
                ("B1432" . ,(lambda () (TO "/offer/~A" (CAAR (FORM-DATA))))))))
    (activate acts)))
                     ...
                     :fields (list
                              ...
                              (list :col "Ресурсы оферты" :perm 111
                                    ...
                                    :fields (list
                                             (list :fld "RESOURCE" :perm 111 :typedata '(:LINK RESOURCE) :name "Ресурс")
                                             (list :fld "PRICE" :perm 111 :typedata '(:NUM) :name "Цена поставщика")
                                             (list :btn "B1433" :perm 111 :value "Удалить из оферты")
                                             (list :btn "B1434" :perm 111 :value "Страница ресурса")))
                              (list :popbtn "P1435"
                                    :value "Добавить ресурс"
                                    :perm 111
                                    :title "Выберите ресурсы"
                                                   ...
                                                   :fields (list
                                                            ...
                                                            (list :popbtn "P1436"
                                                                  ...
                                                                  :fields (list
                                                                           ...
                                                                           (list :btn "B1437" :perm 111 :value "Задать цену"))))))))))))
    (show-acts acts)))

(restas:define-route offer-page/post ("/offer/:id" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
                ("B1433" . ,(lambda () (DEL-INNER-OBJ (CAAR (LAST (FORM-DATA))) *OFFER-RESOURCE*
                                                      (A-RESOURCES (GETHASH (CUR-ID) *OFFER*)))))
                ("B1434" . ,(lambda () (TO "/resource/~A" (CAAR (LAST (FORM-DATA))))))
                ("B1437" . ,(lambda () (LET ((RES-ID (GET-BTN-KEY (CAAR (LAST (FORM-DATA)))))
