        ...
        (acts (list 
               (list :perm ':ALL 
                     ...
                     :fields (list 
                          ...
                          (list :btn "B7227" :perm 111 :value "Показать ресурсы")))))) 
    (show-acts acts)))

(restas:define-route catalog-page/post ("/catalog" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
("B7227" . ,(lambda () (TO "/category/~A" (CAAR (FORM-DATA)))))))) 
       (activate acts)))
...
(restas:define-route category-page ("/category/:id")
  (let ((session (hunchentoot:start-session))
        (acts (list 
               (list :perm ':ALL 
                     ...
                     :fields (list 
                          (list :fld "NAME" :perm 111 :typedata '(:STR) :name "Имя")
                          (list :btn "B7228" :perm 111 :value "Показать ресурсы")))
               (list :perm ':ALL 
                     :grid "NIL"  
 ...
 (CONS-HASH-LIST *RESOURCE*)))
                     :fields (list 
                          ...
                          (list :btn "B7229" :perm 111 :value "Страница ресурса")))))) 
    (show-acts acts)))

(restas:define-route category-page/post ("/category/:id" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
("B7228" . ,(lambda () (TO "/category/~A" (CAAR (FORM-DATA)))))
("B7229" . ,(lambda () (TO "/resource/~A" (CAAR (FORM-DATA)))))))) 
       (activate acts)))
        ...
        (acts (list 
               (list :perm ':ALL 
                     ...
                     :fields (list 
                          ...
                          (list :fld "RESOURCE-TYPE" :perm 111 :typedata '(:LIST-OF-KEYS RESOURCE-TYPES) :name "Тип")
                          (list :fld "UNIT" :perm 111 :typedata '(:STR) :name "Единица измерения")
                          (list :btn "B7230" :perm 111 :value "Страница категории")
                          (list :btn "B7231" :perm 111 :value "Страница ресурса")))))) 
    (show-acts acts)))

(restas:define-route resources-page/post ("/resource" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
("B7230" . ,(lambda () (HUNCHENTOOT:REDIRECT
...
("B7231" . ,(lambda () (TO "/resource/~A" (CAAR (FORM-DATA)))))))) 
       (activate acts)))
        ...
        (acts (list 
               (list :perm ':ADMIN 
                     ...
                     :fields (list 
                          ...
                          (list :btn "B7232" :perm 111 :value "Изменить пароль")))
               (list :perm ':ADMIN 
                     :grid "NIL"  
                     ...
                     :fields (list 
                          ...
                          (list :btn "B7233" :perm 111 :value "Создать новый аккаунт эксперта")))
               (list :perm ':ADMIN 
                     :grid "NIL"  
                     :title "Эксперты"
               ...
               (CONS-HASH-LIST *USER*)))
                     :fields (list 
                          ...
                          (list :popbtn "P7234" 
                                ...
                                :fields (list 
                          (list :btn "B7235" :perm 111 :value "Подтверждаю удаление")))
                          (list :popbtn "P7236" 
                                ...
                                :fields (list 
                          (list :fld "PASSWORD" :perm 111 :typedata '(:PSWD) :name "Пароль")
                          (list :btn "B7237" :perm 111 :value "Изменить пароль эксперта")))
                          (list :btn "B7238" :perm 111 :value "Страница эксперта")))
               (list :perm ':ADMIN 
                     :grid "NIL"  
 ...
 (CONS-HASH-LIST *USER*)))
                     :fields (list 
                          ...
                          (list :popbtn "P7239" 
                                ...
                                :fields (list 
                          (list :btn "B7240" :perm 111 :value "Сделать добросовестным")))))))) 
    (show-acts acts)))

(restas:define-route admin-page/post ("/admin" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
("B7232" . ,(lambda () (LET ((OBJ (CUR-USER)))
...
("B7235" . ,(lambda () (LET ((KEY (GET-BTN-KEY (CAAR (FORM-DATA)))))
  (REMHASH KEY *USER*)
  (HUNCHENTOOT:REDIRECT (HUNCHENTOOT:REQUEST-URI*)))))
("B7237" . ,(lambda () (LET ((OBJ (GETHASH (GET-BTN-KEY (CAAR (LAST (FORM-DATA)))) *USER*)))
  (WITH-OBJ-SAVE OBJ PASSWORD))))
("B7238" . ,(lambda () (TO "/expert/~A" (CAAR (FORM-DATA)))))
("B7240" . ,(lambda () (LET ((KEY (GET-BTN-KEY (CAAR (FORM-DATA)))))
        ...
        (acts (list 
               ...
               (CONS-HASH-LIST *USER*)))
                     :fields (list 
                          (list :fld "NAME" :perm 111 :typedata '(:STR) :name "ФИО")
                          (list :fld "LOGIN" :perm 111 :typedata '(:STR) :name "Логин")
                          (list :btn "B7241" :perm 111 :value "Страница эксперта")))))) 
    (show-acts acts)))

(restas:define-route experts-page/post ("/expert" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
("B7241" . ,(lambda () (TO "/expert/~A" (CAAR (FORM-DATA)))))))) 
       (activate acts)))

(restas:define-route experts-page/ajax ("/G7242")
        ...
        (acts (list 
               ...
               (CONS-HASH-LIST *USER*)))
                     :fields (list 
                          (list :fld "NAME" :perm 111 :typedata '(:STR) :name "Название организации")
                          (list :fld "LOGIN" :perm 111 :typedata '(:STR) :name "Логин")
                          (list :btn "B7243" :perm 111 :value "Страница поставщика")))))) 
    (show-acts acts)))

(restas:define-route suppliers-page/post ("/supplier" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
("B7243" . ,(lambda () (TO "/supplier/~A" (CAAR (FORM-DATA)))))))) 
       (activate acts)))
        ...
        (acts (list 
               (list :perm ':ADMIN 
                     ...
                     :fields (list 
                          (list :fld "LOGIN" :perm 111 :typedata '(:STR) :name "Логин")
                          (list :fld "PASSWORD" :perm 111 :typedata '(:PSWD) :name "Пароль")
                          (list :btn "B7244" :perm 111 :value "Изменить пароль")))
               (list :perm 'NIL 
                     :grid "NIL"  
                     :title "Поставщик"
                     :val (lambda () (GETHASH (CUR-ID) *USER*))
                     :fields (list 
                          ...
                          (list :btn "B7245" :perm 111 :value "Сохранить")
                          ...
                          (list :popbtn "P7246" 
                                ...
                                :fields (list 
                          (list :btn "B7247" :perm 111 :value "Удалить ресурс")))))
                          (list :popbtn "P7248" 
                                :value "Добавить ресурс" 
                                :perm 111 
                                :title "Добавление ресурса" 
                                :fields (list 
                          (list :btn "B7249" :perm 111 :value "Добавить ресурс")))
                          ...
                          (list :btn "B7250" :perm 111 :value "Страница заявки")
                          (list :popbtn "P7251" 
                                :value "Удалить заявку" 
                                :perm 111 
                                :title "Удаление заявки" 
                                :fields (list 
                          (list :btn "B7252" :perm 111 :value "Удалить заявку")))))
                          ...
                          (list :btn "B7253" :perm 111 :value "Страница распродажи")
                          (list :popbtn "P7254" 
                                :value "Удалить распродажу" 
                                :perm 111 
                                :title "Удаление распродажи" 
                                :fields (list 
                          (list :btn "B7255" :perm 111 :value "Удалить распродажу")))))
                          (list :popbtn "P7256" 
                                :value "Добавить распродажу" 
                                :perm 111 
                                :title "Добавление расподажи" 
                                :fields (list 
                          (list :btn "B7257" :perm 111 :value "Добавить распродажу")))))
               (list :perm '(AND :SELF :UNFAIR) 
                     :grid "NIL"  
                     :title "Отправить заявку на добросовестность"
                     :val (lambda () (GETHASH (CUR-ID) *USER*))
                     :fields (list 
                          (list :btn "B7258" :perm 111 :value "Отправить заявку на добросовестность")))))) 
    (show-acts acts)))

(restas:define-route supplier-page/post ("/supplier/:id" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
("B7244" . ,(lambda () (LET ((OBJ (CUR-USER)))
  (WITH-OBJ-SAVE OBJ LOGIN PASSWORD))))
("B7245" . ,(lambda () (LET ((OBJ (GETHASH (CUR-ID) *USER*)))
  ...
  (HUNCHENTOOT:REDIRECT (HUNCHENTOOT:REQUEST-URI*)))))
("B7247" . ,(lambda () (DEL-INNER-OBJ (CAAR (FORM-DATA)) *SUPPLIER-RESOURCE-PRICE*
...
("B7249" . ,(lambda () (PROGN
 ...
 (HUNCHENTOOT:REDIRECT (HUNCHENTOOT:REQUEST-URI*)))))
("B7250" . ,(lambda () (TO "/offer/~A" (CAAR (FORM-DATA)))))
("B7252" . ,(lambda () (DEL-INNER-OBJ (CAAR (FORM-DATA)) *OFFER* (A-OFFERS (GETHASH (CUR-ID) *USER*)))))
("B7253" . ,(lambda () (TO "/sale/~A" (CAAR (FORM-DATA)))))
("B7255" . ,(lambda () (DEL-INNER-OBJ (CAAR (FORM-DATA)) *SALE* (A-SALES (GETHASH (CUR-ID) *USER*)))))
("B7257" . ,(lambda () (CREATE-SALE)))
("B7258" . ,(lambda () (PROGN
  ...
  (let ((session (hunchentoot:start-session))
        (acts (list 
               (list :perm ':ALL 
                     ...
                     :fields (list 
                          (list :fld "NAME" :perm 111 :typedata '(:STR) :name "Распродажа")
                          (list :btn "B7259" :perm 111 :value "Страница распродажи")))))) 
    (show-acts acts)))

(restas:define-route sales-page/post ("/sale" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
("B7259" . ,(lambda () (TO "/sale/~A" (CAAR (FORM-DATA)))))))) 
       (activate acts)))
        ...
        (acts (list 
               (list :perm 'NIL 
                     ...
                     :fields (list 
                          ...
                          (list :btn "B7260" :perm 111 :value "Сохранить")
                          (list :btn "B7261" :perm 111 :value "Удалить распродажу")))))) 
    (show-acts acts)))

(restas:define-route sale-page/post ("/sale/:id" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
("B7260" . ,(lambda () (SAVE-SALE)))
("B7261" . ,(lambda () (DELETE-SALE)))))) 
       (activate acts)))
        ...
        (acts (list 
               ...
               (CONS-HASH-LIST *USER*)))
                     :fields (list 
                          ...
                          (list :fld "LOGIN" :perm 111 :typedata '(:STR) :name "Логин")
                          (list :btn "B7262" :perm 111 :value "Страница застройщика")))))) 
    (show-acts acts)))

(restas:define-route builders-page/post ("/builder" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
("B7262" . ,(lambda () (TO "/builder/~A" (CAAR (FORM-DATA)))))))) 
       (activate acts)))
        ...
        (acts (list 
               (list :perm 'NIL 
                     ...
                     :fields (list 
                          ...
                          (list :btn "B7263" :perm 111 :value "Сохранить")
                          ...
                          (list :btn "B7264" :perm 111 :value "Страница тендера")))))
               (list :perm ':SELF 
                     :grid "NIL"  
                     ...
                     :fields (list 
                          ...
                          (list :btn "B7265" :perm 111 :value "Объявить тендер (+)")))))) 
    (show-acts acts)))

(restas:define-route builder-page/post ("/builder/:id" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
("B7263" . ,(lambda () (LET ((OBJ (GETHASH (CUR-ID) *USER*)))
  (WITH-OBJ-SAVE OBJ NAME JURIDICAL-ADDRESS INN KPP OGRN BANK-NAME BIK
                 CORRESP-ACCOUNT CLIENT-ACCOUNT RATING))))
("B7264" . ,(lambda () (TO "/tender/~A" (CAAR (LAST (FORM-DATA))))))
("B7265" . ,(lambda () (LET ((ID (HASH-TABLE-COUNT *TENDER*)))
        ...
        (acts (list 
               (list :perm ':ALL 
                     ...
                     :fields (list 
                          ...
                          (list :btn "B7266" :perm 111 :value "Страница тендера")))))) 
    (show-acts acts)))

(restas:define-route tenders-page/post ("/tender" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
("B7266" . ,(lambda () (TO "/tender/~A" (CAAR (FORM-DATA)))))))) 
       (activate acts)))
        ...
        (acts (list 
               (list :perm 'NIL 
                     ...
                     :fields (list 
                          ...
                          (list :fld "INTERVIEW" :perm 111 :typedata '(:INTERVAL) :name "Срок проведения интервью")
                          (list :fld "RESULT" :perm 111 :typedata '(:INTERVAL) :name "Срок подведения итогов")
                          (list :btn "B7267" :perm 111 :value "Сохранить")
                          (list :col "Ресурсы тендера" :perm 111 
                                ...
                                :fields (list 
                          (list :fld "NAME" :perm 111 :typedata '(:STR) :name "Название")
                          (list :btn "B7268" :perm 111 :value "Удалить из тендера")
                          (list :btn "B7269" :perm 111 :value "Страница ресурса")))
                          (list :popbtn "P7270" 
                                :value "Добавить ресурс" 
                                :perm 111 
                          ...
                          (list :col "Выберите ресурс" :perm 111 
                                ...
                                :fields (list 
                          (list :fld "NAME" :perm 111 :typedata '(:STR) :name "Наименование")
                          (list :btn "B7271" :perm 111 :value "Добавить ресурс")))))
                          (list :col "Документы тендера" :perm 111 
                                ...
                                :fields (list 
                          (list :fld "NAME" :perm 111 :typedata '(:STR) :name "Название")
                          (list :btn "B7272" :perm 111 :value "Удалить из тендера")
                          (list :btn "B7273" :perm 111 :value "Страница документа")))
                          (list :popbtn "P7274" 
                          ...
                          (list :col "Выберите ресурс" :perm 111 
                                ...
                                :fields (list 
                          (list :fld "NAME" :perm 111 :typedata '(:STR) :name "Наименование")
                          (list :btn "B7275" :perm 111 :value "Добавить ресурс")))))
                          (list :col "Поставщики ресурсов" :perm 111 
                                ...
                                :fields (list 
                          (list :fld "NAME" :perm 111 :typedata '(:STR) :name "Название")
                          (list :btn "B7276" :perm 111 :value "Отправить приглашение")
                          (list :btn "B7277" :perm 111 :value "Страница поставщика")))
                          (list :btn "B7278" :perm 111 :value "Добавить своего поставщика")
                          (list :col "Заявки на тендер" :perm 111 
                                :val (lambda () (CONS-INNER-OBJS *OFFER*
                                                                 (A-OFFERS
                                                                   ...
                                                                   *TENDER*))))
                                :fields (list 
                          (list :btn "B7279" :perm 111 :value "Страница заявки")))
                          (list :popbtn "P7280" 
                                :value "Ответить заявкой на тендер" 
                                :perm 111 
                                :title "Выберите ресурсы" 
                                :fields (list 
                          (list :btn "B7281" :perm 111 :value "Участвовать в тендере")))
                          (list :popbtn "P7282" 
                                :value "Отменить тендер" 
                                :perm 111 
                                :title "Действительно отменить?" 
                                :fields (list 
                          (list :btn "B7283" :perm 111 :value "Подтверждаю отмену")))))))) 
    (show-acts acts)))

(restas:define-route tender-page/post ("/tender/:id" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
("B7267" . ,(lambda () (LET ((OBJ (GETHASH (CUR-ID) *TENDER*)))
  (WITH-OBJ-SAVE OBJ NAME ACTIVE-DATE ALL CLAIM ANALIZE INTERVIEW RESULT))))
("B7268" . ,(lambda () (LET ((ETALON (GETHASH (GET-BTN-KEY (CAAR (LAST (FORM-DATA)))) *RESOURCE*)))
  (SETF (A-RESOURCES (GETHASH (CUR-ID) *TENDER*))
          (REMOVE-IF #'(LAMBDA (X) (EQUAL X ETALON))
                     (A-RESOURCES (GETHASH (CUR-ID) *TENDER*))))
  (HUNCHENTOOT:REDIRECT (HUNCHENTOOT:REQUEST-URI*)))))
("B7269" . ,(lambda () (TO "/resource/~A" (CAAR (LAST (FORM-DATA))))))
("B7271" . ,(lambda () (PROGN
 (PUSH (GETHASH (GET-BTN-KEY (CAAR (LAST (FORM-DATA)))) *RESOURCE*)
       (A-RESOURCES (GETHASH (CUR-ID) *TENDER*)))
 (HUNCHENTOOT:REDIRECT (HUNCHENTOOT:REQUEST-URI*)))))
("B7272" . ,(lambda () (DELETE-DOC-FROM-TENDER)))
("B7273" . ,(lambda () (TO "/document/~A" (CAAR (LAST (FORM-DATA))))))
("B7275" . ,(lambda () (PROGN
 (PUSH (GETHASH (GET-BTN-KEY (CAAR (LAST (FORM-DATA)))) *RESOURCE*)
       (A-RESOURCES (GETHASH (CUR-ID) *TENDER*)))
 (HUNCHENTOOT:REDIRECT (HUNCHENTOOT:REQUEST-URI*)))))
("B7276" . ,(lambda () (DELETE-FROM-TENDER)))
("B7277" . ,(lambda () (TO "/supplier/~A" (CAAR (LAST (FORM-DATA))))))
("B7278" . ,(lambda () (ADD-DOCUMENT-TO-TENDER)))
("B7279" . ,(lambda () (TO "/offer/~A" (CAAR (LAST (FORM-DATA))))))
("B7281" . ,(lambda () (LET* ((ID (HASH-TABLE-COUNT *OFFER*))
       (OFFER
...
("B7283" . ,(lambda () (HUNCHENTOOT:REDIRECT (FORMAT NIL "/tender"))))))) 
       (activate acts)))
        ...
        (acts (list 
               (list :perm ':ALL 
                     ...
                     :fields (list 
                          ...
                          (list :fld "TENDER" :perm 111 :typedata '(:LINK TENDER) :name "Тендер")
                          (list :btn "B7284" :perm 111 :value "Страница заявки")))))) 
    (show-acts acts)))

(restas:define-route offers-page/post ("/offers" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
("B7284" . ,(lambda () (TO "/offer/~A" (CAAR (FORM-DATA)))))))) 
       (activate acts)))
        ...
        (acts (list 
               (list :perm 'NIL 
                     ...
                     :fields (list 
                          ...
                          (list :fld "RESOURCE" :perm 111 :typedata '(:LINK RESOURCE) :name "Ресурс")
                          (list :fld "PRICE" :perm 111 :typedata '(:NUM) :name "Цена поставщика")
                          (list :btn "B7285" :perm 111 :value "Удалить из оферты")
                          (list :btn "B7286" :perm 111 :value "Страница ресурса")))
                          (list :popbtn "P7287" 
                                :value "Добавить ресурс" 
                                :perm 111 
                                :title "Выберите ресурсы" 
                          ...
                          (list :btn "B7289" :perm 111 :value "Задать цену")))))))))))) 
    (show-acts acts)))

(restas:define-route offer-page/post ("/offer/:id" :method :post)
  (let ((session (hunchentoot:start-session))
        (acts `(
("B7285" . ,(lambda () (DEL-INNER-OBJ (CAAR (LAST (FORM-DATA))) *OFFER-RESOURCE*
               (A-RESOURCES (GETHASH (CUR-ID) *OFFER*)))))
("B7286" . ,(lambda () (TO "/resource/~A" (CAAR (LAST (FORM-DATA))))))
("B7289" . ,(lambda () (LET ((RES-ID (GET-BTN-KEY (CAAR (LAST (FORM-DATA)))))
